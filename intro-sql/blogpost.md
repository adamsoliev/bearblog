...
* Cross joins – the `FROM artist AS A, consts AS C` clause logically creates a Cartesian product (cross join) of the two tables.
...
Logical is different than what's happening physically. To figure that out, you have to look at a execution plan (generated by adding `EXPLAIN ANALYZE` to the top of the query), which looks like this:
```
...
Physically, here’s what the database is actually doing:
* Prepares constants — it first runs three sequential scans to find the IDs for *United States*, *English*, and *Person*. These correspond exactly to the subqueries inside the `WITH consts` clause.
